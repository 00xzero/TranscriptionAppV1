...
| 2 | `transcripts.project_id` is `ON DELETE SET NULL (project_id)`. `projects.parent_id` is `ON DELETE NO ACTION`: ordinary parent deletion is blocked, while a set-based delete of the complete branch succeeds because the constraint is checked at statement end. |
...
**Goal:** the database holds projects correctly and defends every invariant without application code. The storage primitives the delete route needs are consolidated in the existing Supabase storage adapter so PR 3 does not depend on PR 2.

### Migration `infra/supabase/migrations/20260912000000_projects.sql`
...
3. **Project triggers.** Three separate trigger functions keep insert, update/marking, and deletion responsibilities explicit. All three are `security invoker` with `set search_path = public`. This is not optional: inside a `security definer` function, `current_user` reports the function's owner, so a role check there would always pass and any client could clear the mark. Invoker triggers see the real caller. Parent lookups work under invoker because the caller either owns the parent (visible under RLS, and `for share` needs the table-wide `UPDATE` privilege `authenticated` holds) or does not, in which case the row is invisible and the trigger raises not-found, which the pair foreign key would also have done. When the definer RPCs run, the triggers see `current_user = postgres` and the privileged paths open.
   ...
   - with a non-null parent: `select deleting_at into v from projects where id = new.parent_id for share`; raise `PJ001` if no row, raise `PJ002` ("project is being deleted") if `v is not null`. Row-level `BEFORE` triggers see rows processed earlier in the same statement, so a two-row insert of A→B and B→A fails on whichever row is processed first, because its parent is not there yet. This is what makes cycles impossible; the foreign key alone is checked at statement end and would pass. The `for share` lock is part of the locking protocol in section 5 below.
   ...
   - if `old.deleting_at is not null` and `new.name is distinct from old.name`: raise `PJ002` (no ordinary edits to a marked project).
   ...
   `projects_before_delete` (`BEFORE DELETE`):
   - if `old.deleting_at is not null` and `current_user not in ('postgres', 'supabase_admin')`, raise `PJ002`. This excludes `service_role`, so only `finish_project_delete` or direct database administration can remove marked projects.
...
5. **Transcript guard and the locking protocol.** One trigger function `transcripts_project_guard`, `security invoker`, `set search_path = public`, fired `BEFORE INSERT OR DELETE OR UPDATE OF project_id, source_object_key, waveform_object_key` on `transcripts`:
   ...
   The capture path's link update runs as the user; the waveform worker's runs as `service_role`. Both hit this guard. PR 3 covers what each does when it fires.
...
9. **Branch helper.** An internal SQL function `project_branch_ids(p_user uuid, p_id uuid) returns uuid[]`:
   ```sql
   with recursive b as (
     select id from projects where id = p_id and user_id = p_user
     union all
     select p.id from projects p join b on p.parent_id = b.id
       where p.user_id = p_user
   ) cycle id set is_cycle using path
   select array_agg(id order by id) from b where not is_cycle;
   ```
   The helper takes the owner explicitly because its callers are `security definer` and cannot rely on RLS. The `CYCLE` clause is defence in depth; the trigger makes cycles unreachable.
...
10. **Two-phase delete RPCs.** Both are `security definer` (owned by `postgres`, so their `UPDATE` of `deleting_at` passes the role check in the update trigger), `set search_path = public`, and both begin with `v_user := auth.uid(); if v_user is null then raise 42501`. Every statement inside filters on `user_id = v_user`; there is no path by which a definer function touches another user's rows. Both are idempotent so a retry can re-run phase one.
    ...
    - `v_ids := project_branch_ids(v_user, p_id)`; raise `PJ001` if null;
    ...
    - in the same transaction, after the loop: `project_branch_inventory(v_user, v_ids)` selects the transcripts with `project_id = any(v_ids) and user_id = v_user` and returns their ids and non-null object keys as sorted, de-duplicated arrays (via `sorted_distinct`). `finish_project_delete` reads the same helper and normalizes its inputs with `sorted_distinct`, so the two sides cannot drift. Because every in-flight membership write has by now either committed or will fail, this inventory is the final one.
    ...
    - resolve the branch again; raise `PJ001` if null; raise `PJ003` ("branch not marked") if any project in it has `deleting_at is null`;
    - raise `PJ004` ("branch changed") if the set of transcripts currently in the branch differs from `p_transcript_ids`, or if the set of non-null `source_object_key` values on those rows differs from `p_media_keys`, or likewise for waveform keys. Compared as sets, not arrays. All three are unreachable given the guard, the delete guard, and the locks; they stay as assertions that make a guard regression loud instead of silent;
    ...
    Transcript deletion cascades to segments, words, speakers, watchlist, and jobs through existing foreign keys; `job_events`, `failed_events`, and `webhook_receipts` set null, all confirmed in current migrations. Plain `NO ACTION` checks at statement end, so deleting every parent and child in one set-based `DELETE` succeeds while deleting a referenced parent alone remains blocked; the smoke script asserts both cases.
...
- `contracts/primitives.ts`: `ProjectNameSchema = z.string().trim().min(1).max(80)`, shared by the project row and write schemas plus the client validator. The DB check constraint mirrors it.
- `contracts/api.ts`: add `project_id: UuidSchema.optional()` to `CreateTranscriptBodySchema`. Add `DeleteProjectResponseSchema` and a stage-discriminated `DeleteProjectErrorSchema`: begin errors may carry `gone`, storage errors require all removal counts, and finish errors may carry `remaining_transcripts`.

### Storage helper extraction

Move `isMissingStorageObjectError` and `removeStorageObjectIfPresent` from `lib/supabase/queries.ts` into `infra/supabase/storage.ts`, the existing storage adapter, and add `removeStorageObjectsBatched(supabase, bucket, keys, batchSize = 100)` which chunks the key list, tolerates missing objects, and returns `{ removed: number, failed: string[] }`. Single-object removal verifies missing responses only for the authenticated user's storage prefix and re-checks ambiguous delete results, so an RLS no-op is not mistaken for successful cleanup. `removed` is derived from the storage API's returned rows rather than the requested batch size. The batch size follows the repo's existing orphaned-media cleanup script. The same module exports `MEDIA_BUCKET`, and the unused, non-tolerant `deleteTranscriptMedia` is deleted.
...
The function's contract is now explicit, because the two halves mean different things to the caller:
...
- **Row deleted**: resolve with `{ cleanupPendingKeys }`, the swept keys whose removal failed. The transcript is gone and stays gone in the UI. When the list is non-empty the surface shows a neutral "Transcript deleted. Some files could not be removed and will be cleaned up later." toast. The recovery path is the orphan cleanup tool. Resolving rather than throwing makes "throw means the transcript still exists" the application-level convention, so the hook's rollback and PR 5's awaited dialog need no special case. It is not a network guarantee: a response lost after the delete committed still throws, which is why the hook reconciles after rolling back and a retry treats an absent row as success.
- **`maybeSingle` returns null**: another tab already deleted the row. That is a successful end state; resolve with no pending keys.

`useTranscriptsRealtime.deleteTranscript` in `lib/supabase/hooks.ts` returns the result. On any throw it restores only the removed transcript, captured synchronously from the hook's current data because a state updater may run after the request settles. The row is re-inserted in `created_at` order and only if it is not already present, so concurrent realtime changes survive; the hook then refetches to reconcile a delete that committed but lost its response. `__tests__/deleteTranscript.test.ts` gains cases for a key that appears between read and delete, sweep failure reporting pending keys, and an already-deleted row resolving normally. `__tests__/supabaseHooks.test.tsx` gains the pass-through case, a local rollback case, and a rollback that preserves a concurrent realtime insert.
...
- direct `delete from projects` on a parent rejected by `NO ACTION`; direct delete of a leaf nulls only `project_id` on its transcripts and leaves `user_id` intact;
- `begin_project_delete` marks a three-level branch and returns complete arrays; calling it again on the already-marked branch succeeds and returns the same arrays; moving a transcript into or out of the marked branch is rejected with `PJ002`; inserting a transcript with `project_id` in the branch is rejected; creating a project under it is rejected; renaming a marked project is rejected; updating `source_object_key` or `waveform_object_key` on a transcript in the branch is rejected, both as `authenticated` and as `service_role`; `finish_project_delete` removes all projects and transcripts and returns correct counts; `finish` with a stale id list raises `PJ004`; `finish` without `begin` raises `PJ003`; both RPCs on a foreign id raise `PJ001`;
- as `authenticated`, `update projects set deleting_at = null` and `... = now()` both raise `42501`; the same as `postgres` succeeds; direct deletes of a transcript or project in a marked branch raise `PJ002` as `authenticated` and as `service_role`, and `finish_project_delete` still succeeds afterwards;
- inserting a transcript whose `project_id` names no row raises `PJ001` (not `23503`); inserting into a marked project raises `PJ002`. These two assertions pin the codes `isProjectGoneError` depends on;
- `finish_project_delete` with a key array that omits one inventoried key raises `PJ004`;
...
The single-session smoke script cannot exercise two transactions at once, and the locking protocol is only proven by overlap. Add `pg`, `@types/pg`, and `tsx` as dev dependencies and drive two connections against the local database, each authenticated as the same user via `set local role authenticated` and the JWT claims setting. Five interleavings, each asserted:
...
2. Connection B calls `begin_project_delete(P)` inside an open transaction (rows locked, uncommitted). Connection A attempts a move into P. Assert A blocks. B commits. Assert A fails with `PJ002`.
...
5. Connection A begins a transaction and deletes a transcript in P (open). Connection B calls `begin_project_delete(P)`. Assert B blocks. A commits. Assert B's `transcript_ids` excludes it. Then, with P marked, a fresh delete of another transcript in P on connection A fails with `PJ002`.
...
- `__tests__/projectsMigration.test.ts`: extract and assert each function definition independently. Assert `nulls not distinct`, the pair foreign keys, plain `on delete no action`, `on delete set null (project_id)`, the parent-exists check and `for share` in the insert trigger, the role check on `deleting_at` in the update trigger, that every new trigger function is `security invoker`, the transcript guard's event list including `delete`, `security definer` plus `auth.uid()` in each RPC, the fixed-point loop in `begin_project_delete`, the `cycle` clause, exactly the three intended grants to `authenticated`, that no cancel function exists, and the publication add.
...
- `validateProjectName(name)`: calls `ProjectNameSchema.safeParse` from `contracts/primitives.ts` and maps the first issue to a user-facing message. No independent rule copy.

### `lib/supabase/project-errors.ts`

- `mapProjectWriteError(error)`: Postgres `23505` to the duplicate-name message, `23503` to "that project no longer exists", `PJ001` to not-found, `PJ002` to "that project is being deleted", generic fallback otherwise. Lives in the query layer because it interprets driver errors, which is not domain logic. Exports `isProjectGoneError(error)`, true for `PJ001` (the guard's not-found, which fires before the foreign key and is therefore the error a missing project normally produces), `PJ002` (marked for deletion), and `23503` (the foreign key, reachable only if the guard is bypassed). The create fallback in PR 6 uses it, and the smoke script proves the actual code each case raises.
...
A context provider mounted once in `app/layout.tsx` inside the existing providers. It owns one `useAuthIdentity()` call and calls `useProjectsRealtime({ userId, enabled: Boolean(userId) })` and `useTranscriptsRealtime({ userId, enabled: Boolean(userId) })` directly, so authentication changes never replace the provider element or remount application children. The hooks bind all state and callbacks to a monotonic committed scope epoch, synchronously exposing empty loading state on an account transition and rejecting or ignoring work captured by an older scope instance. Without a user id both hooks remain mounted but disabled; the provider reports loading while auth is unresolved and settled empty data once the absence of a session is known.

The provider publishes two independently memoised contexts. `useProjectsData()` exposes only projects, the tree, project loading/error/connection state, and project create/rename/mutate/refetch actions. `useTranscriptsData()` exposes only transcripts, transcript loading/error/connection state, and transcript delete/move/add/mutate/refetch actions. Mixed consumers read both hooks. This keeps one auth listener and one realtime subscription per table while ensuring an update in one domain does not rerender consumers of only the other domain. Other app subsystems retain their pre-existing auth lookups and listeners.

With no user, the provider calls both table hooks in their disabled state, so they issue no fetch and open no channel; the provider tests assert that.

### Tests

- `__tests__/projects/tree.test.ts`, `activity.test.ts`, `validate.test.ts`, `projectErrors.test.ts`.
- `__tests__/projects/provider.test.tsx`: one hook instance per table regardless of consumer count; both context slices reach their consumers; pending-auth loading; disabled table hooks with no user; callbacks pass through without wrappers.
...
3. rpc('begin_project_delete', { p_id })
     PJ001 -> 404 { stage: 'begin', gone: true }
...
5. rpc('finish_project_delete', { p_id, p_transcript_ids, p_media_keys, p_waveform_keys })
   passing back exactly the arrays begin returned
     PJ004 (branch changed) -> 409 { stage: 'finish' }, retry re-runs step 3
...
- a `begin` failure touches neither storage nor `finish`; `PJ001` maps to 404 with `gone: true`;
...
- **Waveform worker** (`lib/inngest/functions/handle-waveform-requested.ts`): the link update appends `.select('id').single()` so a zero-row update surfaces as an error. What happens next depends on why the link failed, because a link can commit and still lose its response, and removing the object then would leave a live transcript pointing at a missing waveform:
  - `PJ002`, or a confirmed zero-row result: the link did not happen. Remove the object just uploaded (the key is deterministic via `buildWaveformObjectKey`, so this is one `remove` call) and throw `NonRetriableError`.
...
- `__tests__/inngestHandlers.test.ts`: extend for zero-row detection, removal on `PJ002`, reconciliation treating a committed link as success, removal after reconciliation shows a different key, and no storage action when the re-read fails.
...
- `app/projects/page.tsx` (client): reads `useProjectsData()` and `useTranscriptsData()`. Renders root project rows, an Unfiled section header with count, then unfiled transcript rows via `transcriptsInProject(transcripts, null)`. Empty state when there are no projects and no unfiled transcripts.
...
Before the sidebar and dialogs broaden use of the shared provider, profile it with a realistic large transcript set. Measure the initial fetch plus the reconciliation fetch after realtime connects, polling-fallback traffic, and cross-domain rerenders. Preserve one provider and one subscription per table; keep project and transcript values in separate domain contexts.

Provider-refactor checkpoint result: with 250 projects and 2,000 transcripts, the synthetic initial load issues one project fetch and one transcript fetch; realtime reconciliation and polling fallback each issue one additional fetch per table. A project update rerenders project and mixed consumers but not transcript-only consumers, and a transcript update rerenders transcript and mixed consumers but not project-only consumers. Tests intentionally make no assertion about rerenders within a consumer's own domain.
...
- `core/transcripts/create.ts`: accept `project_id`. The insert is wrapped so that an error satisfying `isProjectGoneError` (`PJ001` from the guard because the project no longer exists, `PJ002` because it is marked for deletion, or `23503` from the foreign key) retries the same insert once with `project_id: null` and sets `warning: 'project_missing'` on the result. The retry goes through the existing insert branch, so a `23505` on `upload_intent_id` during the retry still resolves to the existing row exactly as it does today. `runCaptureUpload` surfaces the warning; the capture modal and recording page show a toast ("Saved to Unfiled: the project is no longer available"). The recording recovery path exercises this end to end: a session persisted with a project id whose project was deleted before recovery lands Unfiled with the warning.
- `__tests__/createTranscript.test.ts`: extend for all three fallback codes and for the fallback-plus-dedupe race. The smoke script in PR 1 is what proves the database actually raises `PJ001` and `PJ002` on those paths; this unit test proves the code reacts to them.
