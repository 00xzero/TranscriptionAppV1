# V3 Targeted Rewrite — Combined Scope
...
> [!NOTE]
> Billing integration is out of scope for V3. The goal is to establish the domain model and interfaces so billing can be plugged in later without architectural changes.
...
| Segment split/merge in editor               | Existing planned feature, not V3 architecture concern              |
...
4. **Webhook large-payload strategy**: do we enforce a strict recording-duration cap in V3, or support a non-Vercel callback ingress for long recordings immediately?
